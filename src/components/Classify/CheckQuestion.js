...
const SubAnswer = (props) => {
    ...
    var question = fund.questions.find(
        (x) =>
            x.questionpath === props.questionpath &&
            x.id === props.parentquestion
    );
    ...
    return (
        <div className="ifyesdiv">
            ...
            <div
                className="row rowofcs"
                id={
                    "subanswercontainer_" +
                    props.questionpath.replace("/", "_") +
                    "_" +
...
const CheckQuestion = observer((props) => {
    ...
    if (fund.questions) {
        if (fund.questions.find((x) => x.questionpath === props.questionpath)) {
            hasAnswers = fundStore.funds
                .find((x) => x._id === id)
                .questions.filter((x) => x.questionpath === props.questionpath)
                .length;
        }
    }

    const storeCheckboxResult = (
        questionpath,
        name,
        answer,
        currentcheckstatus
        ...
        if (currentcheckstatus) {
            ...
            const q = fund.questions.find(
                (x) => x.questionpath === questionpath && x.id === name
            );
    ...
    const getExistenceOfAnswer = (qid) => {
        ...
        if (fund.questions) {
            const qs = fund.questions.find(
                (x) => x.questionpath === props.questionpath && x.id === qid
            );
            if (qs) return true;
        }
